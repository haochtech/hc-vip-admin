...
function arraytoxml($data){
    ...
    global $_GPC, $_W;
        ...
        curl_setopt($ch,CURLOPT_SSLCERT,IA_ROOT . "/addons/zh_vip/cert/".'apiclient_cert_' . $_W['uniacid'] . '.pem'); //这个是证书的位置绝对路径
        curl_setopt($ch,CURLOPT_SSLKEY,IA_ROOT . "/addons/zh_vip/cert/".'apiclient_key_' . $_W['uniacid'] . '.pem'); //这个也是证书的位置绝对路径
...
$system=pdo_get('zhtc_system',array('uniacid'=>$_W['uniacid']));
    $data=array(
        ...
        'partner_trade_no'=>date("YmdHis",strtotime($list['time'])),//商户订单号
   ...
   // print_r($WxPayApi->SetSign($key));die;
    ...
    if($return['result_code']=='SUCCESS'){
      $res=pdo_update('zhtc_withdrawal',array('state'=>2,'sh_time'=>date('Y-m-d H:i:s')),array('id'=>$id));  
