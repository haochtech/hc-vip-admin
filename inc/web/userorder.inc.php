...
if($_GPC['user_id']){
	$count = pdo_fetchcolumn("SELECT COUNT(*) FROM". tablename("zhvip_order"). "  where UNIX_TIMESTAMP(time)>=".$dcstart." and unix_timestamp(time)<=".$dcend."  and uniacid=".$_W['uniacid']." and state=2 and  user_id=".$_GPC['user_id']);
